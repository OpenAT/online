...
""")
    ...
    # ACTIONS
    # TODO: Make this a server action also with a scheduled call
    @api.multi
    def action_check_widget(self):
        ...
        try:
            target_page = requests.get(self.target_url, timeout=10)
        except:
            errors.append(_("Could not fetch %s") % self.target_url)
        ...
        # TODO: add correct source_url to the xpath
        ...
        # TODO: Also test target URL widget integration (need to write website_url_manager first)
    ...
    # source
    # Todo: add a many2one field source_url which points to website_url_manager entries
    #       with a domain [('redirect_url', 'not', Null)]
